...
import tempfile, os, pandas as pd, re, numpy as np
...
tool = st.sidebar.selectbox("Choose Tool", [
    "1. DNA ↔ RNA Conversion", "2. RNA → Protein Translation", "3. Mutation Detection",
    "4. Codon Usage Analysis", "5. Sequence Alignment (Global)", "6. FASTA/FASTQ Parser",
    "7. Reverse Complement", "8. GC Content & Melting Temp", "9. Restriction Enzyme Sites",
    "10. ORF Finder", "11. Nucleotide Frequency Chart", "12. Central Dogma (DNA->RNA->Protein)",
    "13. Hamming Distance", "14. Molecular Weight Calculator", "15. Motif Finder (Pattern Search)",
    "16. Advanced Graphing & Visualization", "17. CRISPR-Cas9 Cut Site & Efficiency",
    "18. Phylogenetic Distance Matrix", "19. PCR Primer Designer",
    "20. Translation Table Selector", "21. Sequence Statistics"
])
st.sidebar.markdown("---")
...
if tool == "1. DNA ↔ RNA Conversion":
    ...
    with col1:
        ...
        if st.button("Convert to RNA", key="btn1"):
            if dna_input:
                ...
                st.success(f"RNA: {rna_output}"); st.info(f"Length: {len(rna_output)} bases")
    ...
    with col2:
        ...
        if st.button("Convert to DNA", key="btn2"):
            if rna_input:
                ...
                st.success(f"DNA: {dna_output}"); st.info(f"Length: {len(dna_output)} bases")
            else: st.warning("Please enter a sequence first.")
    ...
    if st.button("Translate to Protein", key="btn3"):
        if rna_seq:
            try:
                seq_obj = Seq(rna_seq.upper()); protein = seq_obj.translate(table=table_map[table], to_stop=False)
                st.success("Protein Sequence Generated!"); st.code(str(protein), language="text")
    ...
    if st.button("Detect Mutations", key="btn4"):
        if original and mutated:
            ...
            if len(original) != len(mutated): mutations.append(f"Length Difference: {len(original)} vs {len(mutated)} bp")
            if mutations:
    ...
    if st.button("Align Sequences", key="btn6"):
        if seq1 and seq2:
            try:
                aligner = PairwiseAligner(); aligner.mode = 'global'; aligner.match_score = 2
                aligner.mismatch_score = -1; aligner.open_gap_score = -2; aligner.extend_gap_score = -0.5
                best = aligner.align(seq1.upper(), seq2.upper())[0]
                st.success(f"Alignment Score: **{best.score}**"); st.text(best.format())
            except Exception as e: st.error(f"Error: {str(e)}")
        else: st.warning("Please enter both sequences.")
    ...
    if st.button("Calculate GC & Tm", key="btn8"):
        if dna_seq:
            dna_seq = dna_seq.upper().replace('U', 'T'); length = len(dna_seq)
            gc_percentage = GC(dna_seq); gc_count = dna_seq.count('G') + dna_seq.count('C')
        ...
        if dna_seq:
            ...
            for enzyme, site in enzymes.items():
                if site in dna_seq:
                    ...
                    st.write(f"**{enzyme}** ({site}): Positions {positions}"); found = True
    ...
    if st.button("Find ORFs", key="btn10"):
        if dna_seq:
            try:
                seq_obj = Seq(dna_seq.upper()); orfs = seq_obj.translate(to_stop=False).split('*')
                ...
                for i, orf in enumerate(valid_orfs[:5], 1):
                    st.write(f"ORF #{i}: {len(orf)} amino acids"); st.code(str(orf)[:50] + "...", language="text")
            except Exception as e: st.error(f"Error: {str(e)}")
        else: st.warning("Please enter a sequence first.")
    ...
    if st.button("Run Central Dogma", key="btn12"):
        if dna_seq:
            dna_seq = dna_seq.upper(); rna = dna_seq.replace('T', 'U'); protein = Seq(rna).translate(to_stop=False)
            st.info(f"**DNA:** {dna_seq}"); st.success(f"**RNA:** {rna}"); st.write(f"**Protein:** {protein}")
        else: st.warning("Please enter a sequence first.")
    ...
    if st.button("Calculate Weight", key="btn14"):
        if seq_input:
            length = len(seq_input.strip()); weights = {"DNA": 650, "RNA": 340, "Protein": 110}
    ...
    if graph_type == "Amino Acid Composition":
        ...
        if st.button("Generate Composition Chart", key="btn_aa"):
            if protein_seq:
                protein_seq = protein_seq.upper().replace('*', ''); aa_counts = {}
        ...
        if st.button("Generate GC Graph", key="btn_gc"):
            if dna_seq:
                dna_seq = dna_seq.upper().replace('U', 'T'); gc_values, positions = [], []
                for i in range(0, len(dna_seq) - window_size + 1, 10):
                    window = dna_seq[i:i+window_size]; gc_percentage = GC(window)
                    gc_values.append(gc_percentage); positions.append(i + window_size//2)
    ...
    if st.button(" Predict CRISPR Cut Sites", key="btn_crispr"):
        if target_dna:
            seq = target_dna.upper().replace('U', 'T'); regex_pam = ".GG"
            matches = [(m.start(), m.group()) for m in re.finditer(f'(?={regex_pam})', seq)]
            if not matches: st.warning("No NGG PAM sites found in the sequence.")
            else:
                st.success(f"Found {len(matches)} potential PAM site(s).")
                for idx, (pos, pam) in enumerate(matches):
                    if pos >= 20:
                        protospacer = seq[pos-20:pos]; score = 50
                        gc_count = protospacer.count('G') + protospacer.count('C'); gc_pct = (gc_count / 20) * 100
                        ...
                        tier = "HIGH" if score >= 70 else "MODERATE" if score >= 40 else "LOW"; cut_pos = pos - 3
                        with st.expander(f"Site #{idx+1}: Position {pos+1} (Score: {score}% - {tier})"):
                            st.markdown(f"**Protospacer:** `{protospacer}`"); st.markdown(f"**PAM:** `{pam}`")
                            st.markdown(f"**Cut Site:** Between bp {cut_pos} and {cut_pos+1}")
                            st.write(f"**GC Content:** {gc_pct:.1f}% | **Efficiency:** {score}%")
        else: st.warning("Please enter a target DNA sequence.")
    ...
    if fasta_file:
        try:
            with tempfile.NamedTemporaryFile(delete=False, suffix='.fasta') as tmp:
                tmp.write(fasta_file.getvalue()); tmp_path = tmp.name
            records = list(SeqIO.parse(tmp_path, "fasta")); n = len(records)
            if n < 2: st.warning("Please upload at least 2 sequences.")
            else:
                ...
                for i in range(n):
                    for j in range(i+1, n):
                        ...
                        dist = mismatches / min_len; dist_matrix[i, j] = dist_matrix[j, i] = dist
                df_dist = pd.DataFrame(dist_matrix, index=[r.id for r in records], columns=[r.id for r in records])
                st.write("### Distance Matrix (P-distance)"); st.dataframe(df_dist.round(4))
                csv = df_dist.to_csv().encode('utf-8')
                st.download_buton_button("📥 Download Distance Matrix CSV", csv, "distance_matrix.csv", "text/csv")
            os.unlink(tmp_path)
        except Exception as e: st.error(f"Error: {str(e)}")
    ...
    if st.button("🎯 Design Primers", key="btn_primer"):
        if template:
            ...
            with col1:
                ...
                if abs(fwd_tm - target_tm) <= 3: st.success("✅ Optimal Tm")
                else: st.warning("️ Tm deviation")
            with col2:
                st.subheader("Reverse Primer (5'→3')")
                st.code(rev_primer, language="text")
                st.write(f"Tm: {rev_tm:.1f}°C | GC: {rev_gc:.1f}%")
                if abs(rev_tm - target_tm) <= 3: st.success("✅ Optimal Tm")
                else: st.warning("⚠️ Tm deviation")
            st.info(f"Expected PCR Product Size: ~{len(template)} bp")
        else: st.warning("Please enter a template sequence.")
    ...
    genetic_tables = {"1 - Standard": 1, "2 - Vertebrate Mitochondrial": 2, "3 - Yeast Mitochondrial": 3,
        "4 - Mold Mitochondrial": 4, "5 - Invertebrate Mitochondrial": 5, "11 - Bacterial": 11,
        "12 - Alternative Yeast": 12, "13 - Ascidian Mitochondrial": 13, "14 - Alternative Flatworm": 14,
        ...
        if seq_input:
            try:
                ...
                if table_id != 1:
                    ...
                    if str(protein) != str(std_protein): st.info("⚠️ This genetic code produces different amino acids than Standard code")
            except Exception as e: st.error(f"Error: {str(e)}")
        else: st.warning("Please enter a sequence.")
    ...
    if st.button("Analyze Statistics", key="btn_stats"):
        if seq_input:
            ...
            with col1: st.metric("Length", f"{length} {'bp' if seq_type != 'Protein' else 'aa'}")
            with col2:
                if seq_type != "Protein":
                    gc_percentage = GC(seq)
            ...
            if seq_type != "Protein":
                ...
                if seq_type == "DNA":
                    ...
                    else: st.warning("⚠️ Sequence may be single-stranded or have bias")
            else:
